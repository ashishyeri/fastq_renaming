...
INSTALL_ENV="$CONFIG_DIR/install_paths.env"
{
  ...
  echo "# Then run 'uv sync' so the project venv is created at FASTQ_PIPELINE_ROOT/.venv"
  echo ""
  echo "# uv: create .venv in storage path (not project dir)"
  echo "export UV_PROJECT_ENVIRONMENT=\"$ROOT/.venv\""
  echo ""
